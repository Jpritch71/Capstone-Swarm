...
public abstract class E_GridedMovement : TaggableEntity
{
    ...
    //This is an abstact class, so these don't get run ever.
    //Intended to be a template for what actual implementations will do.
    void Awake()
    {        
        InitAwake();
    }
    ...
    {
        StartCoroutine(StartPathToNode(WorldManager._WORLD.GetTile(targetIn.Pos)));
    }
    ...
    protected virtual IEnumerator StartPathToNode(Node nodeIn)
	{
        ...
        if (workingNode.Walkable)
        {
            ...
            if (workingNode != WorldManager._WORLD.GetTile(transform.position))
            {
                ...
                E_GridedMovement charMove = this;
    ...
    protected virtual void GridMovement()
    {
        ...
        if (wayPointNodes != null && wayPointNodes.Count > 0)
        {
            ...
            if (CurrentTargetNode != null)
            {
                ...
                Pos = transform.position + (targetPositionOFFSET - Pos).normalized * (Time.deltaTime * Speed);
    ...
    #region components
    public GroupController squad { get; private set; }
